...
Use the same `X_CLIENT_ID` and `X_CLIENT_SECRET` names in ignored `.env.production.local` for the production X app. The production callback is `https://www.doom-or-bloom.com/api/auth/callback/twitter`, matching the canonical `BETTER_AUTH_URL`. Register that exact URL in the X app: the bare-domain redirect does not make the two callback URLs interchangeable. Development credentials remain in `.env.development.local`.
