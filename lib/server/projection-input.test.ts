...
test('lossless aliases retain complete raw observations and restore original provenance', () => {
  ...
  const question = {
    ...
    criteria: {
      'original-answer-49:e': state.answers[49]!.text,
      none: 'Unsupported'
    }
  }
  ...
  expect(compact.input.completeParticipantEvidence).toHaveLength(50)
  ...
  expect(result.type === 'choice' && result.choice).toBe('original-answer-49:e')
  expect(
    result.type === 'choice' && result.probabilities['original-answer-49:e']
  ).toBe(1)
})
