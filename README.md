...
You can complete an assessment, publish its full conversation and inferred results, make it private again, or continue in an independent private fork. Shared pages and WebP previews check current visibility. External sites may retain previously fetched previews. [The implementation plan](docs/persistence-implementation-plan.md) records integrated acceptance and successful local X sign-in, anonymous claim and recovery. Database-backed personas and optional account claims are implemented. TypeSafe’s data policies apply to Jev processing.
