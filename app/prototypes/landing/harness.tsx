...
import { QuestionFirst } from './question-first'
import { MapFirst } from '@/components/landing/map-first'
import { PeopleFirst } from './people-first'
import type { Example } from '@/components/landing/shared'
import './picker.css'
import '@/components/landing/landing.css'

const variants = [QuestionFirst, MapFirst, PeopleFirst]
const names = ['Question', 'Map', 'People']
  ...
  return (
    <>
      <div className='landing-stage'>
        <Variant
          key={`${active}-${replay}`}
          examples={active === 1 ? examples : examples.slice(0, 3)}
          variant={active + 1}
        />
      </div>
      <nav
