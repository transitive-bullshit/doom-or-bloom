...
- Hosted PostgreSQL URLs should explicitly use `sslmode=verify-full` to verify both the certificate and hostname. The shared database URL configuration normalizes pg 8's legacy `prefer`, `require`, and `verify-ca` aliases to `verify-full`, preserving existing verification and removing the driver's major-version compatibility warning. Explicit `uselibpqcompat=true` settings are retained; local URLs without SSL settings are unchanged. This code change takes effect after deployment; it does not update Vercel environment variables itself.
...
## Persona query and index release — September 26, 2026

- [x] Commit the query/index changes as `0db150f9`; the featured-map social image is separately committed as `1bc828b0`.
- [x] Apply migration `0006_sloppy_maginty` to production `neondb` through its direct connection, with a five-second lock timeout and 60-second statement timeout. All seven recorded migration hashes match the repository. The account identity, fork source-assessment/source-snapshot, retry, and corrected owner-library indexes are valid.
- [x] Execute the new summary queries against production through the normal application pool: 143 summaries / 4,449,497 serialized bytes; 44 featured summaries / 1,612,534 bytes. Production homepage returned 200 after migration. No snapshot data or account uniqueness rules were changed.

Release validation includes the prior checkpoint's passing local migrations, database checks, eight persistence browser cases, and 297 core tests. The fresh core run passed again (Vitest 2.67s). The full 63-case browser run initially had 51 passes and 12 failures (5.4m): nine stale assertions and three absent local journey records. Corrected nine assertions to use the library breadcrumb, original portrait export source, and current authored catalog count; all nine then passed (35.6s). Analytics/privacy passed all three cases (20.6s). The restart check initially matched both header and main CTAs; it now scopes the start link to the main page.

Three unrelated internal `/user-journeys` tests remain unverified: mobile uncertainty/paperclip paths, the fixed real-user transcript, and completed uncertain-persona maps. The latest ignored local run lacks those fictional/fixed-user records, so the inspector correctly reports no saved run. These tests still need deterministic local fixtures; no paid generation was performed to populate them. This inspector is unavailable in production, and its file-backed loader does not use the changed Postgres persona repository. This release does not claim a fully green local browser suite.

The corrected restart check passed: fresh native database, repeat migrations/seed, killed POST, process restart, restored input, and one explicit retry. `pnpm build:local` passed with all 143 profiles and two public assessments pregenerated. `pnpm check:public-cache` passed (one case, 3.0s); `pnpm check:prefetch` passed (seven cases, 8.3s). These final checks used `0db150f9` plus the documented test-only corrections. The user explicitly authorized the production migration and Git production push; preview migration remains outside this release.
