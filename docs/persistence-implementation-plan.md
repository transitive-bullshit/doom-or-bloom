...
### Public-page ISR and mobile header — September 24, 2026

- [x] Cache `/`, `/about`, and curated `/users/<slug>` HTML with a 24-hour ISR interval. Prerender landing/About at build time and generate persona paths on demand, with request-time server APIs rejected. Session controls remain client-side; participant routes and social-image policies are unchanged.
- [x] Hide the anonymous header CTA and its loading placeholder below the `sm` breakpoint (640 px), retaining the signed-in account menu. Update the existing header regression at 320, 390, 639, 640 and 1488 px.
- [x] Verify local production behavior and document freshness: CLI/database persona changes appear after the next successful time-based regeneration or deployment, without immediate invalidation.

Validation on base `06a7953` with this checkpoint's changes: `pnpm test` passed (55 files / 270 tests, formatting, lint, types, content and unused-code checks); `pnpm check:browser tests/browser/header-account.spec.ts` passed 2 cases; `pnpm db:test:personas` passed; `pnpm check:persistence tests/persistence/personas.spec.ts` passed 1 case; `pnpm build:local` passed compilation and trace checks. The final build contains `initialRevalidateSeconds: 86400` for landing/About and blocking on-demand persona generation. A temporary production server through Portless returned `HIT` for landing/About and `MISS` then `HIT` with identical HTML for `/users/barackobama`, all with `s-maxage=86400`. A delayed-session browser check confirmed the mobile header stays 76 px tall before/after anonymous session resolution at 320/390 px and keeps the signed-in account menu visible at 390 px. Screenshots and headers are in `/tmp/doom-cache-verification/`. No deployment or production mutation was performed.

### Public social-image background refresh — September 24, 2026

- [x] Replace `must-revalidate` with `stale-while-revalidate=86400` in the shared successful public-image response policy. Participant and persona previews remain fresh for seven days and can serve stale content for one additional day while refreshing.
- [x] Update privacy copy, llms.txt, and the persistence contract to disclose up to eight days of cached-image availability after revocation. This supersedes the seven-day total retention allowance above; origin publication checks and private export policies remain unchanged.
...
## Profile portrait loading — September 25, 2026

- [x] Load the above-the-fold profile portrait eagerly with high fetch priority. Show a map-sized Next image until the larger portrait has decoded, using the same 40-pixel image props as the featured and directory maps so their browser-cache entries can be reused.
- [x] Verify the placeholder remains visible during a delayed full-size request, then disappears after decoding, at both 1x and 2x screen density.

Validation on `02f0d8e7` plus this checkpoint's dirty tree: `pnpm test` passed all 297 cases and core checks; `pnpm build:local` passed with all 141 profiles and both local public assessments pregenerated. `pnpm check:prefetch` passed all seven cases in 9.6s with Postgres unreachable. The new cases navigate from each map, hold the larger image response, verify the placeholder's selected URL exactly matches the map portrait, and check eager/high-priority loading and the decoded-image swap. Pending and loaded screenshots were visually inspected. The shared profile header also gives published participant portraits eager loading; social-card and map-export code is unchanged.

## Profile social-preview compatibility — September 25, 2026

- [x] Reproduce the reported missing X preview without sending a message or publishing a post. Production returned complete metadata and a valid 1200 × 630 WebP; X's validator reported success, but its post composer displayed a generic icon. The homepage JPEG displayed correctly. A fresh profile URL crawl reproduced the missing image. Production image requests returned 200, including cache hits.
- [x] Serve profile social cards as PNG and advertise a new image URL with matching MIME metadata. Retain the existing seven-day image cache policy, public assessment WebP route, browser portrait optimization, and PNG map/download renderers.
- [x] Verify the built route, image bytes, initial metadata, and existing navigation checks locally.
- [x] Confirm the image is visibly rendered in X after the automatic deployment. The PNG change is a compatibility mitigation; HTTP success and validator success alone do not prove X displays an image.

Validation on `21d3851c` plus this checkpoint's dirty tree: `pnpm test` passed all 297 cases, including native profile PNG rendering, participant WebP rendering, and download parity. `pnpm build:local` passed with all 141 profiles and both local public assessments pregenerated. `pnpm check:prefetch` passed all seven cases in 9.7s with Postgres unreachable. A request to the built Roon profile verified matching Open Graph/Twitter URLs in the initial head, `image/png` metadata, and a 200 response containing a decodable 1200 × 630 PNG (96,282 bytes) with the existing public cache headers; the image was visually inspected. No production records or external messages were created.

Hosted verification of `89ba9cb8`: the automatic Git deployment served matching PNG metadata and the same decodable 96,282-byte image. X's unsent post composer visibly rendered full 1200 × 630 image cards for Mira and for `https://www.doom-or-bloom.com/users/tszzl?v=png-1`, using images fetched into X's own image CDN. The original Roon URL still displayed its older icon-only preview after validator refresh, so the versioned page URL is the verified immediate sharing workaround. This establishes that the new profile cards render in X; it does not isolate image-format compatibility from X's image/card caching. Existing DM previews were not resent or verified. Temporary drafts were discarded, and no posts or DMs were sent.

## Joscha Bach and Vittorio additions — September 26, 2026

- [x] Ground two separately labeled simulated users in inspected primary sources, with dated research notes and portraits.
- [x] Generate only their local live interviews and selected results; verify existing profiles and immutable snapshots are preserved.
- [x] Verify persona persistence, local profile presentation, and repository checks; retain selected public payloads for the subsequent production import.

The user subsequently authorized production import, commit and push. Generated results remain ignored local data; a Git push alone does not import them.

Validation on `33410f66` plus this checkpoint’s working tree: `pnpm test` passed all 297 cases plus format, lint, types, content and unused-code checks; `pnpm db:test:personas` passed; the selected-profile persistence browser case passed; `pnpm build:local` verified 143 pregenerated profiles; all seven `pnpm check:prefetch` checks passed, including every built profile with Postgres unavailable. Both local profile layouts and generated answers were inspected. Full unrelated participant/auth/analytics release suites were not rerun for this source-and-profile addition.

Bach has 19 source records (11 interviews/writings/papers and eight X posts); Vittorio has 12 original X records including one authored essay. They are unfeatured directory entries, preserving the existing 44 featured users. Bach’s live run `1790357274147-3e682f3b-c70d-4a0a-9aa0-19b2194d71e4` accepted one answer; Vittorio’s `1790357319136-75184095-6a9d-4735-af04-9f2c3fc7b348` accepted two. Both stopped automatically under existing readiness/routing. Three participant requests and 15 Jev requests cost an estimated $0.0678 combined, with no remaining reservations.

Observed outputs: Bach outlook 74.25/100, tentative transformation 53.75/100 (full 0–100 interpretation range), inferred P(doom) ≈1%; Vittorio outlook 75/100, transformation 63.54/100, inferred P(doom) <1%. These are generated assessment interpretations, not either person’s stated probabilities, and no public P(doom) overrides were added. Bach’s tentative transformation is retained rather than tuned toward an expected placement.

Direct checksum comparisons confirmed all 141 prior profile records/selections and all 631 prior immutable snapshots unchanged. Exact selected source snapshots match the new authored briefs. The ignored `work/research/bach-vittorio/selected-simulations.json` contains only these two validated public simulations, their metadata and source briefs for the later production import; no participant/auth records are included. Production remained unchanged during local validation. The subsequent authorized import is recorded in [production readiness](production-readiness.md#joscha-bach-and-vittorio-import--september-26-2026).

## Zuckerberg interview refresh — September 26, 2026

- [x] Add the original September 24 Joanna Stern interview using inspected primary captions, with speaker-scoped grounding and a local source preview.
- [x] Regenerate only Zuckerberg’s local selected simulation and document observed result deltas without inventing a stated probability.
- [x] Verify the selected brief and preservation of all other profiles and prior snapshots; leave unrelated database schema and query work out of the commit.

[Research, source timestamps and result comparison](research/zuckerberg-interview-2026-09-26.md) record the source addition (five to six), successful two-answer run, and preceding transient upstream HTTP 503. `pnpm test` passed all 297 cases plus format, lint, types, content and unused-code checks. `pnpm db:test:personas` and the selected-profile persistence browser check passed. `pnpm build:local` verified 143 pregenerated profiles; all seven prefetch checks passed with the database unavailable. Verification used the shared working tree, which also contained unrelated database schema/query work; those changes are excluded from this checkpoint. Only local Zuckerberg data was refreshed; no production update was performed.

## Persona query payloads and lookup indexes — September 26, 2026

- [x] Project list-view metadata, sources and selected results in SQL; filter featured users before fetching their snapshots. Preserve exact results, source drift, selection ordering, and full single-profile interviews.
- [x] Add account provider/account lookup and source-assessment/source-snapshot/retry deletion indexes. Replace the owner library's `updated_at` ordering with `created_at`.
- [x] Apply migration `0006_sloppy_maginty` to native local test Postgres twice and development Postgres once; verify repository, lifecycle, auth, build and browser behavior.

Validation on the working tree based on `f7af979c` (unrelated concurrent source/metadata changes excluded from this checkpoint): `pnpm test` passed all 297 cases across 61 files (Vitest 2.75s), plus format, lint, types, content and unused-code checks. `pnpm db:migrate:test` passed twice, followed by `db:test`, `db:test:repository`, `db:test:commit`, `db:test:lifecycle`, `db:test:personas`, and `db:test:auth`; database-suite elapsed time was not recorded. Extended persona checks compare every summary with its full snapshot, featured-only selection, both historical/live payload formats, and changed source briefs without changing frozen recorded sources. Development migration passed. The initial connection attempt timed out in Postgres.app's authentication permission check; connections succeeded after opening the app, and all migration/database checks then passed.

`pnpm check:persistence` passed all eight cases in 45.6s. `pnpm build:local` passed and verified all 143 pregenerated profiles and two local public participant assessments. `pnpm check:prefetch` passed all seven cases in 8.8s with the database unavailable. No UI changes required visual design review; full unrelated browser/analytics/restart release suites were not run.

A read-only local comparison of serialized repository results measured 71,547,879 bytes for 143 full records versus 4,449,497 bytes for 143 summaries (93.8% smaller); featured summaries returned 44 records / 1,612,534 bytes. One sequential local run measured 751ms, 229ms and 49ms respectively, including parsing; these are observations, not a production latency benchmark. SQL projection still reads/decompresses the underlying JSONB, but no longer transmits the full histories. Existing immutable snapshots and cache lifetimes remain unchanged.

No paid inference, hosted migrations, push or deployment performed. Apply the checked-in migration explicitly to hosted databases when preparing their release. Account indexing is nonunique, preserving existing authentication behavior; optional latest-operation and persona-pointer indexes remain deferred because observed query costs were negligible.

## Local admin oversight and Drizzle Studio — September 26, 2026

- [x] Add a read-only `/admin` overview, filtered/paginated assessment and owner lists, saved conversation/results inspection, frozen published snapshot selection, operation/recovery metadata, and the participant library preview using shared components and shadcn primitives.
- [x] Default metrics to participant records and all time. Document first-submission persistence, creation cohorts, provisional results, inherited answers, ownership transfer, deleted records, and the distinction between database owners and visitors.
- [x] Enforce development/build, runtime, local host, hosted-environment and tunnel guards. Block admin routes before filesystem routing in production artifacts; use an independently configured read-only database pool without application-pool fallback.
- [x] Add explicit local/production admin and Studio commands. Keep normal development running independently with the `admin.doom-or-bloom` Portless route and `.next-admin` directory. Use existing direct Neon credentials for inspection; pooled startup options are unsupported.
- [x] Verify core checks, browser inspection, persistence/privacy regressions, production routing denial, and actual local/production read-only database settings. Start the production-backed dashboard and production Studio locally for handoff.

Validation on `b8c9732a` plus this checkpoint's working tree: `pnpm test` passed all 304 cases across 64 files (latest Vitest command 2.86s), formatting, lint, type generation/typechecking, content validation, and unused-code checks. `pnpm check:browser tests/browser/admin.spec.ts` passed (16.3s including setup), covering synthetic saved results, private inspection without owner cookies, creation-date/status filtering, user library preview without mutation controls, local/forwarded-host denial, mobile drawer navigation, and unchanged persisted revisions. Desktop, mobile, and result screenshots were visually reviewed; result layout is contained within the admin column. `pnpm check:persistence` passed all eight cases in 40.3s, and `pnpm check:analytics` passed all three in 17.5s.

`pnpm build:local` passed: 143 pregenerated profiles, two local cached public assessments, existing trace/portrait checks, and the new production admin rewrite assertion. A local production server with `ADMIN_ENABLED=true`, `LOCAL_ADMIN_BUILD=true`, and an unreachable admin database returned plain 404/private-no-store responses for all six tested admin HTML/RSC paths. The final additional tunnel guards and independent admin launcher passed core tests and the actual production-backed development smoke check. The production dashboard loaded real participant aggregates successfully in the browser. Native local Postgres and direct production Neon metadata queries verified read-only mode and the 15-second timeout without retrieving participant text. Both Studio launchers started on loopback; production Studio uses the verified direct connection.

Next 16's development renderer overrides configured cache-control with `no-cache, must-revalidate`; this is recorded in [admin documentation](admin.md), and the browser test checks actual behavior. Schema, migration, production mutation, paid inference, deployment, and push were not part of this checkpoint. Existing full unrelated browser, database transaction, restart, and public-cache suites were not rerun. No production data was modified.

## PostgreSQL SSL compatibility warning — September 26, 2026

- [x] Preserve pg 8's full certificate and hostname verification by normalizing legacy `prefer`, `require`, and `verify-ca` URL modes to explicit `verify-full` in shared database configuration. Retain explicit libpq compatibility settings and local connections without SSL options.
