...
### Experimental worldview views — `worldview-v3`
...
Inferred P(doom) uses the complete worldview, including indirect priorities and expected outcomes. Jev classifies probability bands; code combines their midpoints and widens interpretation ranges for indirect or missing evidence. It does not report Jev confidence as catastrophe probability. Verification checks whether the excerpt supports the interpretation in context, without requiring an explicit number. See [assessment rules](ASSESSMENT.md).
