...
The [revised editorial packet](current-context-review-packet.md) records approved authoring direction and demo priority. New assessments use the `0.3.0-draft` bundle with 135 references and 14 recommendations; saved `0.2.0-draft` assessments retain their earlier 42-reference corpus. The [coverage index](research/source-coverage-2026-09-17.md) links all required originals, current copies, authoring scopes and remaining gaps. Individual factual/semantic review remains open. Source intake records scoped research for all required URLs; access and corpus review gates remain open.
