...
Use the global heading styles in `app/globals.css` on every route, including landing pages, assessment owner/public pages, personas, informational pages, and local tools. At the default root size, h1–h6 are 30, 24, 20, 18, 16, and 14px respectively, with weight 600, line-height 1.4, and balanced wrapping. The scale stays consistent across breakpoints. Choose heading levels for page/section hierarchy; do not add local text-size, weight, leading, or tracking overrides. Heading classes may control layout, spacing, alignment, and color. Component labels such as bookmark titles remain independent non-heading elements.
