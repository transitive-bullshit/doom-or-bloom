...
Image provenance is retained in `lib/sharing/resource-previews.json`; unresolved URLs are recorded in [resource-preview-gaps.json](resource-preview-gaps.json). The latest pass covers 199 of 207 non-tweet URLs, including preserved older snapshot URLs. Remaining gaps include blocked/time-out sources and obsolete snapshot links; the UI keeps normal bookmarks for these rather than fabricated imagery.
