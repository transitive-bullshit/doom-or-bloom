...
After adding or changing source URLs, run `pnpm resources:previews` before committing. This covers current briefs as well as recorded resources and stores resized WebP images and icons under `public/resource-previews/`, indexed by `lib/sharing/resource-previews.json`. Source summaries travel to profile bookmarks as descriptions, ahead of generic site metadata.

Preview preference: publisher social/article image, publisher YouTube oEmbed thumbnail (including embedded episode videos), reviewed page capture or document page, then a locally rendered title/domain card. Missing publisher favicons receive an explicitly recorded site-initial monogram. Title cards and monograms are inexpensive deterministic fallbacks, not invented publisher artwork, and later refreshes can replace them. Browser captures can be supplied with `--captures=<directory>` using the hashed URL filenames; review them to exclude challenges, broken pages, cookie overlays and blank renders. `--concurrency=1` is available for image runtimes that abort under concurrent decoding.

The source-preview regression check requires each current non-tweet source to have a local image, icon and description. X posts retain the tweet presentation. Discovery directories such as independent.prose.md belong in intake provenance, not opinion-grounding source lists.
