...
Sharing is available from the results view only. Intermediate projections saved while answering do not make an interview publishable; the API also rejects publication until the assessment is in a results, completed, or capped state. There is no separate Done button on assessment detail.
