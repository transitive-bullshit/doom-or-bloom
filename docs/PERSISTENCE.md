...
Status: implemented and locally verified on 2026-09-23, including participant persistence, forks, publication, curated database personas and optional X ownership claims. [The implementation plan](persistence-implementation-plan.md) records integrated acceptance and real local X login/recovery evidence. This document supersedes the browser-only persistence, no-account, destructive-restart, and generic-only sharing restrictions in the original MVP handoff. Production Neon migrations, persona seeding, and Vercel environment preparation are complete; deployment and hosted callback/crawler verification remain separate work.
...
The landing page, About page, and curated `/users/<slug>` profiles use shared server-rendered HTML with a 24-hour ISR interval (`revalidate = 86400`). Landing and About are prerendered at build time; persona profiles are generated on first request, including slugs added after a deployment. After the interval, the next request can receive the previous page while regeneration refreshes it. Persona selection/source changes made by CLI or database operations become visible on the next successful regeneration or a fresh deployment; they do not trigger immediate invalidation. These routes reject request-time server APIs with `dynamic = 'error'`; account/session controls resolve separately on the client. Participant public/private assessment routes retain their request-time visibility checks and existing cache policies.
...
Generate a 1200 × 630 WebP social image with the installed Takumi renderer, extending `lib/sharing/social-card.tsx` and its existing WebP options. An explicit route such as `/public/assessments/<id>/social-image.webp` reads the final snapshot and current visibility; metadata uses its absolute URL and `image/webp`. A normal participant card uses its actual result and neutral assessment title, without a synthetic label or automatic X identity. Persona cards retain their simulated label and portrait. Keep existing participant PNG downloads available.

Public assessment HTML is rendered on each request, with indexable canonical metadata, Open Graph/Twitter previews, full answer text, and result disclosures in the initial HTML. Client hydration enables interactions. HTML and JSON remain uncached and check current visibility. Successful public Takumi image GET responses (participant and persona previews) are cached by browsers and shared caches for seven days; the accepted tradeoff is that cached images can remain accessible for that period after unpublishing or deletion. Origin requests still deny unpublished/deleted content. Private PNG export POST responses and errors remain no-store. Render on demand without image hosting or a new service. External social networks may retain previews already fetched; making an assessment private cannot erase their copies. Verify generated bytes, dimensions, headers, contrast, long-title behavior, unknown placements, and metadata linkage; defer actual external crawler verification to a publicly reachable deployment.
