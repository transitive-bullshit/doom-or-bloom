...
The library uses a shadcn data table with separate sortable creation-date and status columns, defaulting to newest created first. Assessment titles open the private detail page; Published badges open the public page. Management actions live in each row’s dropdown, with confirmation before deletion. Published rows also offer View public assessment and Copy public link in the menu. There is no pagination, filtering, selection, or column chooser.
