...
| --- | --- | --- | --- |
| A. Interpret | Current prompt/answer, relevant usable history, shared context, alias candidates | Response disposition, substantiveness, expressed worldview distinctions, epistemic evidence, ambiguity, topic families, candidate reference identity | Gate recovery first; only usable answers contribute validated judgments/evidence |
| B. Resolve references, conditional | Shortlisted IDs/labels, then selected reviewed reference summaries and the current whole answer | Identity when topic widening was needed; attribution, evidentiary fit, uncertainty, materiality | Separate mentioned references from contextual retrieval; grounding evidence with provenance |
...
Multiple references can occur in one answer: use independent candidate-presence judgments over the whole answer instead of forcing the entire answer into one reference Choice. Bound candidates and resolved entries per turn; if coverage is incomplete, retain an unresolved-reference marker and offer clarification. Unknown references and uncited plain-language reasoning must not automatically lower epistemic quality.
