...
API failures emit JSON logs through console.error (5xx) or console.warn (expected 4xx), visible in local server output and Vercel function logs. Responses include X-Request-ID; assessment stage and Jev batch failures carry the same server-generated ID. Logs include route, status, elapsed time, stage, request/question sizes, selected context-field sizes, attempts and whether an overflow split will retry. Error summaries retain type, safe provider request ID/status, stack frames and bounded causes, without raw answers, credentials, provider bodies or exception messages. Search by request ID to correlate a failed browser request with its stages. Jev max_tokens_exceeded retries split question batches to a maximum depth of two under the existing attempt/time budgets; complete answers are never truncated.
