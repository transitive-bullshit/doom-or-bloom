...
API failures emit JSON logs through console.error (5xx) or console.warn (expected 4xx), visible in local server output and Vercel function logs. Responses include X-Request-ID; assessment stage and Jev batch failures carry the same server-generated ID. Logs include route, status, elapsed time, stage, request/question sizes, selected context-field sizes, attempts and physical call failures. Error summaries retain type, safe provider request ID/status, stack frames and bounded causes, without raw answers, credentials, provider bodies or exception messages. Search by request ID to correlate a failed browser request with its stages. Jev transient failures retry once. A max_tokens_exceeded response now fails the operation without recursive batch splitting; complete answers are never truncated.

Failure logs also carry the API handler phase (`parse_input`, `evaluate`, `validate_output`, rendering, fetch or storage), abort status, deployment ID, commit and region when available. A fingerprint groups matching error types/codes and stack frames within the same build. Underlying Node network/filesystem codes, provider HTTP status, timeout/abort categories and wrapper causes help distinguish transport failures from validation bugs. Assessment errors include operation type, answer/prompt/evidence counts and submitted/saved answer byte sizes. Client input validation is a 400 warning; server output validation is a 500 error. In Vercel Runtime Logs, filter by the failed route and search the response’s `X-Request-ID`; inspect `api_request_failed`, `assessment_stage_failed` and `jev_batch_failed` together. Use the deployment/commit fields to identify the exact code version. No debug switch is required.
