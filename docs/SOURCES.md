...
All 114 required records now link to scoped research notes in the intake registry. That records what was accessible and actually inspected; it does not claim full-text access or corpus completion. Intake statuses are 67 scoped drafts, 29 research drafts, 17 partial-access records and one blocked record. Ninety-nine required URLs map to active or separate draft snapshots; none of these mappings records new editorial approval.
...
The [measurement/disclosure batch](research/required-measurement-batch-01.md) adds 15 separate drafts and maps 12 required URLs, retaining three original developer hubs alongside separately pinned September cards. Dashboard success rates, evidence vintages, inherited assessments, private test conditions and organizational disclosures remain explicit. Separate draft snapshots now total 115; the current demo stays at 42 references. [Workforce follow-up](research/required-workforce-evidence.md) further distinguishes descriptive payroll signals from randomized workplace interventions and preserves the blocked exact HBS working-paper version; it is research, not asset review.

The [workforce batch](research/required-workforce-batch-01.md) adds six scoped snapshots, retaining causal versus descriptive estimands, percentage-point versus relative-percent differences and intervention/evidence vintages. Four related-version mappings retain partial access for the exact required HBS/NBER originals; a mapped source is not necessarily completed. No new asset is individually reviewed or activated.
...
The [governance/control/scenario batch](research/required-governance-batch-01.md) adds 11 scoped drafts, including a [pinned RSP policy follow-up](research/required-rsp-policy-followup.md). Written provisions, conditional scenarios and ontologies remain distinct from compliance or demonstrated capability. The [social/video batch](research/required-social-batch-01.md) adds nine verified-root or selected-caption publications, retaining unverified empirical claims, quote/edit dependencies and attribution limits. These batches remain outside the demo and unreviewed.
